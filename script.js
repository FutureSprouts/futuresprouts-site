...
<header class="navbar">
  ...
  function footerHtml() {
    ...
    const socialDefs = [
      ["Instagram", cfg.socials && cfg.socials.instagram],
      ["Facebook", cfg.socials && cfg.socials.facebook],
      ["X", cfg.socials && cfg.socials.x],
      ["LinkedIn", cfg.socials && cfg.socials.linkedin],
      ["YouTube", cfg.socials && cfg.socials.youtube],
      ["TikTok", cfg.socials && cfg.socials.tiktok]
    ].filter(([, url]) => url);
    const socialsHtml = socialDefs
...
<div class="cart-line">
  ...
  // ---------------------------
  // Testimonials + Gallery (config-driven; hidden until populated)
  // ---------------------------
  function renderTestimonials() {
    const slot = document.getElementById("testimonials");
    if (!slot) return;
    const items = (cfg.testimonials || []).filter(t => t && t.quote);
    if (!items.length) { slot.style.display = "none"; return; }
    slot.style.display = "";
    const grid = slot.querySelector(".testimonial-grid");
    if (!grid) return;
    grid.innerHTML = items.map(t => `
      <figure class="card testimonial-card reveal">
        <blockquote>${escapeHtml(t.quote)}</blockquote>
        <figcaption>
          ${t.photo ? `<img loading="lazy" src="${escapeHtml(t.photo)}" alt="${escapeHtml(t.name || "")}" class="testimonial-photo">` : ""}
          <div>
            <strong>${escapeHtml(t.name || "")}</strong>
            ${t.role ? `<div class="small">${escapeHtml(t.role)}</div>` : ""}
          </div>
        </figcaption>
      </figure>`).join("");
  }
  ...
  function renderGallery() {
    const slot = document.getElementById("photoGallery");
    if (!slot) return;
    const items = (cfg.galleryPhotos || []).filter(p => p && p.src);
    if (!items.length) { slot.style.display = "none"; return; }
    slot.style.display = "";
    const grid = slot.querySelector(".gallery-grid");
    if (!grid) return;
    grid.innerHTML = items.map(p => `
      <figure class="gallery-item reveal">
        <img loading="lazy" src="${escapeHtml(p.src)}" alt="${escapeHtml(p.alt || "")}">
        ${p.caption ? `<figcaption class="small">${escapeHtml(p.caption)}</figcaption>` : ""}
      </figure>`).join("");
  }
  ...
  // ---------------------------
  // Delivery map pins (Donate page)
  // ---------------------------
  function renderMapPins() {
    const layer = document.getElementById("mapPins");
    if (!layer) return;
    const pins = (cfg.deliveryPins || []).filter(p => p && p.x != null && p.y != null);
    const note = document.getElementById("mapEmptyNote");
    if (!pins.length) { if (note) note.style.display = ""; return; }
    if (note) note.style.display = "none";
    layer.innerHTML = pins.map((p, i) => `
      <div class="map-pin" style="left:${Number(p.x)}%; top:${Number(p.y)}%; animation-delay:${i * 120}ms;" title="${escapeHtml(p.label || "")}">
        <span class="map-pin-dot" aria-hidden="true"></span>
        ${p.label ? `<span class="map-pin-label">${escapeHtml(p.label)}</span>` : ""}
      </div>`).join("");
  }

  // ---------------------------
  // Meet the farm (About page)
  // ---------------------------
  function renderFarmAnimals() {
    const slot = document.getElementById("farmAnimals");
    if (!slot) return;
    const items = (cfg.farmAnimals || []).filter(a => a && a.name);
    if (!items.length) { slot.style.display = "none"; return; }
    slot.style.display = "";
    const grid = slot.querySelector(".animal-grid");
    if (!grid) return;
    grid.innerHTML = items.map(a => `
      <figure class="animal-card reveal">
        ${a.photo ? `<img loading="lazy" src="${escapeHtml(a.photo)}" alt="${escapeHtml(a.name)} the ${escapeHtml(a.type || "farm animal")}">` : ""}
        <figcaption>
          <strong>${escapeHtml(a.name)}</strong>${a.type ? ` <span class="small">· ${escapeHtml(a.type)}</span>` : ""}
          ${a.fact ? `<div class="small">${escapeHtml(a.fact)}</div>` : ""}
        </figcaption>
      </figure>`).join("");
  }

  // ---------------------------
  // Hero ambient: drifting seeds (skipped for reduced motion)
  // ---------------------------
  function initHeroSeeds() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      seed.className = "seed";
      seed.style.left = (8 + Math.random() * 84) + "%";
      seed.style.animationDuration = (14 + Math.random() * 10) + "s";
      seed.style.animationDelay = (-Math.random() * 20) + "s";
      seed.style.setProperty("--drift", (Math.random() * 60 - 30) + "px");
      seed.style.setProperty("--scale", (0.6 + Math.random() * 0.7).toFixed(2));
  ...
  function initScrollGrowth() {
    const svg = document.getElementById("growthPlant");
    if (!svg) return;
    const stem = svg.querySelector(".growth-stem");
    ...
    function setProgress(p) {
      stem.style.strokeDashoffset = len * (1 - p);
      stages.forEach(el => {
        el.classList.toggle("grown", p >= parseFloat(el.dataset.stage));
      });
    }
    ...
    function onScroll() {
      ...
      requestAnimationFrame(() => {
        ...
        setProgress(max > 0 ? Math.min(1, window.scrollY / (max * 0.85)) : 1);
        ticking = false;
      });
    }
  ...
  // ---------------------------
  // Seasonal theming (divider art + accents follow the calendar)
  // ---------------------------
  function initSeason() {
